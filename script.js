function printpdf() {
  ...
  function addtr() {
    ...
    head.innerHTML = ('<table class="edu_tab" id="table"><tr class="bor_du parent"><th class="bor_du" contenteditable="true">Sr. No.</th><th class="bor_du" contenteditable="true">Name</th><th class="bor_du" contenteditable="true">Degree</th><th class="bor_du" contenteditable="true">CGPA</th><th class="bor_du" contenteditable="true">Year</th></tr></table>');
    saveresume();
  }
  function remtr(event) {
    let val = 0;
    const allInputCheckboxes = event.target.parentElement.getElementsByClassName("parent");
    const array = Array.from(allInputCheckboxes);
    if (array.length === 0) {
        alert("No fields are present to be deleted!")
    }
